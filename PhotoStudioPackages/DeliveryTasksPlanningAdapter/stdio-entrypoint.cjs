...
const { planDeliveryTasksFromSnapshot } = require('./index.cjs');
...
function handleRequest(request) {
  ...
  try {
    return success(request, planDeliveryTasksFromSnapshot(request.payload));
  } catch (errorValue) {
...
function emit(response) {
  ...
  const fallbackLine = `${JSON.stringify(failure(response, 'OUTPUT_LIMIT_EXCEEDED', 'Response exceeds byte limit.'))}\n`;
