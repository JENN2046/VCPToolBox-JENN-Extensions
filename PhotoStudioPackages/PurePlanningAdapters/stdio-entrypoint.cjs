...
const SAFE_PROJECTION_ERROR_CODES = Object.freeze([
  'PRONOUN_REGISTER_CONFLICT'
]);

const ACTION_TABLE = Object.freeze({
  [ACTIONS.caseContentDraft]: Object.freeze({
    creationId: CREATION_IDS.caseContentDraft,
    build: buildCaseContentDraftFromSnapshot
  }),
  [ACTIONS.replyDraft]: Object.freeze({
    creationId: CREATION_IDS.replyDraft,
    build: buildClientReplyDraftFromSnapshot
  }),
  [ACTIONS.deliveryPriority]: Object.freeze({
    creationId: CREATION_IDS.deliveryPriority,
    build: buildDeliveryPriorityFromSnapshot
  })
});
...
function validateRequest(request) {
  ...
  if (typeof request.action !== 'string' || request.action === '*' || request.action.includes('*')) return failure(request, 'ACTION_DENIED', 'Wildcard action is not authorized.');
  ...
  const spec = ACTION_TABLE[request.action];
  if (request.creationId !== spec.creationId) return failure(request, 'CREATION_ID_DENIED', 'Creation id is not authorized for this action.');
...
function handleRequest(request) {
  ...
  try {
    return success(request, ACTION_TABLE[request.action].build(request.payload));
  } catch (errorValue) {
    const candidateCode = errorValue && typeof errorValue.code === 'string' ? errorValue.code : null;
    const code = SAFE_PROJECTION_ERROR_CODES.includes(candidateCode) ? candidateCode : 'PROJECTION_REJECTED';
    return failure(request, code, errorValue && errorValue.message ? errorValue.message : 'Projection rejected input.');
  }
}
