...
const { buildSelectionNoticeFromSnapshot } = require('./index.cjs');
...
function handleRequest(request) {
  ...
  try {
    return success(request, buildSelectionNoticeFromSnapshot(request.payload));
  } catch (errorValue) {
...
function main() {
  ...
  process.stdin.on('end', () => {
    ...
    if (!Buffer.from(text, 'utf8').equals(inputBytes)) {
