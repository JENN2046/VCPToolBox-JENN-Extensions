...
const ENTRY = path.resolve(__dirname, '../PhotoStudioPackages/FollowupReminderPlanningAdapter/stdio-entrypoint.cjs');
const LIMIT = 262144;
const BASE = {"protocolVersion":1,"requestId":"synthetic-request","creationId":"jenn.photo-studio.plugin-followup-reminder","action":"plan_followup_reminder_from_snapshot","payload":{"referenceDate":"2026-06-24","reminderType":"quotation_followup","projectSnapshot":{"project_id":"synthetic-current","status":"quoted"},"existingReminderSnapshots":[]}};
...
test('different illegal byte IDs cannot enter domain comparison', async () => {
  const request = clone(BASE); request.payload.projectSnapshot.project_id = '__CURRENT__'; request.payload.existingReminderSnapshots = [{ project_id: '__EXISTING__', reminder_type: 'quotation_followup', status: 'pending' }];
...
for (const [name, value] of [
  ...
  test(`valid ${name} retains request identity and successful result`, async () => {
    ...
    assert.equal(response.result.executionAuthorized, false); assert.equal(response.result.notificationAuthorized, false); assert.equal(response.result.requiresHumanReview, true);
  });
}
