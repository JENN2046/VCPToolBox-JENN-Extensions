...
const ROOT = path.resolve(__dirname, '..', 'PhotoStudioPackages', 'ProjectTasksPlanningAdapter');
const MAX = 262144;
const SOURCE = Object.fromEntries(['stdio-entrypoint.cjs', 'index.cjs', 'projectTaskPlanProjection.cjs'].map(name => [name, fs.readFileSync(path.join(ROOT, name), 'utf8')]));
function runMain(input) {
  ...
  const requireBound = name => {
    if (name !== './index.cjs' && name !== './projectTaskPlanProjection.cjs') throw new Error('UNADMITTED_IMPORT');
...
function validRequest(requestId = 'fixture-request') {
  return { protocolVersion: 1, requestId, creationId: 'jenn.photo-studio.plugin-project-tasks', action: 'plan_project_tasks_from_snapshot', payload: { projectSnapshot: { project_id: 'fixture-project', project_type: 'portrait' } } };
}
