...
const ENTRY = path.resolve(__dirname, '../PhotoStudioPackages/ReadOnlyApiAdapters/stdio-entrypoint.cjs');
const LIMIT = 262144;
const BASE = {"protocolVersion":1,"requestId":"synthetic-request","creationId":"jenn.photo-studio.plugin-delivery-operator-report","action":"build_delivery_operator_report_from_snapshots","payload":{"referenceDate":"2026-06-24","commandCenterSnapshot":{},"deliveryReadinessSnapshot":{}}};
...
test('different illegal byte IDs cannot enter domain comparison', async () => {
  const request = clone(BASE); request.payload.deliveryReadinessSnapshot = { deliveryId: '__CURRENT__' }; request.payload.commandCenterSnapshot = { previews: { deliveries: [{ id: '__EXISTING__', status: 'ready' }] } };
...
for (const [name, value] of [
  ...
  test(`valid ${name} retains request identity and successful result`, async () => {
    ...
    assert.equal(response.result.fullParity, false);
  });
}
