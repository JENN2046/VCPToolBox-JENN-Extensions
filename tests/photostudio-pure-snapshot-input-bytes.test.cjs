...
const LIMIT = 262144;
const BASE = {"protocolVersion": 1, "requestId": "pure-synthetic-boundary", "creationId": "jenn.photo-studio.plugin-case-content-draft", "action": "build_case_content_draft_from_snapshot", "payload": {"generatedAt": "2026-06-24T01:02:03.000Z", "tone": "warm", "contentItemSnapshot": {"content_item_id": "CONTENT-SYN-001", "project_id": "PRJ-SYN-001", "customer_id": "CUST-SYN-001", "customer_name": "Studio Client", "project_name": "Campaign Still Set", "project_type": "commercial", "theme": "bright editorial layout", "deliverables_summary": "Eight selected images are ready for publication.", "usage_status": "portfolio_review"}}};
...
test('different corrupt raw IDs cannot collapse into a replacement-character identity', async () => {
  assertDecodeRejected(await run([withRawId(Buffer.from([0xff]))]));
  assertDecodeRejected(await run([withRawId(Buffer.from([0xfe]))]));
});
...
for (const [name, value] of [
  ...
  test(`valid ${name} retains request identity and successful result`, async () => {
    ...
    assert.equal(response.result.fullParity, false); assert.equal(response.result.syntheticOnly, true); assert.equal(response.result.publishReady, false); assert.equal(response.result.requiresHumanReview, true); assert.equal(response.result.automaticPublicationAuthorized, false);
  });
}
