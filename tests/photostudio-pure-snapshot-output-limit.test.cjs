...
const MAX = 262144;
const SOURCE = Object.fromEntries(['stdio-entrypoint.cjs', 'index.cjs', 'deliveryPriorityProjection.cjs', 'caseContentDraftProjection.cjs', 'replyDraftProjection.cjs'].map(name => [name, fs.readFileSync(path.join(ROOT, name), 'utf8')]));
function runMain(input) {
  ...
  const requireBound = name => {
    if (!['./index.cjs', './deliveryPriorityProjection.cjs', './caseContentDraftProjection.cjs', './replyDraftProjection.cjs'].includes(name)) throw new Error('UNADMITTED_IMPORT');
...
function validRequest(requestId = 'fixture-request') {
  return { protocolVersion: 1, requestId, creationId: 'jenn.photo-studio.plugin-delivery-priority', action: 'prioritize_delivery_actions_from_snapshot', payload: { referenceDate: '2026-06-24', externalExportSnapshots: [] } };
}
test('ordinary IDs and successful advisory payload are unchanged', () => {
  ...
  assert.equal(r.frame.result.recommendationSemantics, 'ADVISORY_ONLY');assert.equal(r.frame.result.fullParity, false);assert.ok(r.outputBytes < MAX);
});
test('first bounded OUTPUT_LIMIT fallback retains IDs when it fits', () => {
  const input = validRequest('x'.repeat(MAX - 350));const r = runMain(JSON.stringify(input) + '\n');
